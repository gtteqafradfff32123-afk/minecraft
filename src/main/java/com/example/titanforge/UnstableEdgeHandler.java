...
public final class UnstableEdgeHandler {
    ...
    public void onHurt(LivingHurtEvent event) {
        ...
        float normalMin = new float[]{0, 1.10F, 1.15F, 1.25F}[level];
        float normalMax = new float[]{0, 1.25F, 1.40F, 1.60F}[level];
        float critMin = new float[]{0, 1.25F, 1.40F, 1.60F}[level];
        float critMax = new float[]{0, 1.45F, 1.75F, 2.10F}[level];
        float min = realCrit ? critMin : normalMin;
        float max = realCrit ? critMax : normalMax;
        float multiplier = min + player.getRNG().nextFloat() * (max - min);
        ...
        if (multiplier >= (realCrit ? 1.55F : 1.35F)) {
            LivingEntity target = event.getEntityLiving();
            ((ServerWorld) player.world).spawnParticle(
                    realCrit ? ParticleTypes.ENCHANTED_HIT : ParticleTypes.CRIT,
