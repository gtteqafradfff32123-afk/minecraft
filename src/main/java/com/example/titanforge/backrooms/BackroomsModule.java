...
public abstract class BackroomsModule {
    ...
    private AxisAlignedBB boundsCache;
    ...
    public BackroomsModule(BlockPos origin, Direction facing, int width, int depth, int height) {
        this.origin = origin;
        this.facing = facing;
        this.width = width;
        this.depth = depth;
    ...
    protected final void socket(int x, int y, int z, Direction dir) {
        sockets.add(new ModuleSocket(origin.add(x, y, z), dir));
    }
    ...
    public void place(ServerWorld world, BlockPos center) {
        ...
        for (DelayedBlock db : blocks) {
            BlockPos wp = new BlockPos(ox + db.x, oy + db.y, oz + db.z);
            world.setBlockState(wp, db.state, 3);
        }
